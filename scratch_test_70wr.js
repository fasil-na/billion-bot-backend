...
async function testConfig(rr, gap, minRisk) {
    ...
    const res = {
        ...
        json: (data) => {
            ...
            netProfit = simulatedTrades.reduce((acc, t) => acc + t.profit, 0);
            ...
            console.log(`RR: ${rr.toFixed(1)} | Gap: ${gap} | Risk: ${minRisk} | WR: ${overallWinRate.toFixed(1)}% | Net: $${netProfit.toFixed(0)} | Profits: ${p.map(x=>x.toFixed(0)).join(', ')} | WRs: ${w.join(', ')}`);
        }
    };
...
async function run() {
    await testConfig(2.0, 0.0006, 50);
    await testConfig(2.0, 0.0008, 60);
    await testConfig(2.2, 0.0008, 60);
    await testConfig(1.8, 0.0008, 60);
}
run();
